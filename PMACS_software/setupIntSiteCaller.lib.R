runIntSiteCaller <- function(outputDir, refGenome, s){
  ...
  message('runIntSiteCaller(', outputDir, ', ', refGenome, ', s)')
  ...
  invisible(sapply(r, function(x){
    ...
    if(grepl('_I1_', x)){
      system(paste0('scp ', remoteHost, ':', x, ' ', paste0(outputDir, '/Data/Undetermined_S0_L001_I1_001.fastq.gz')))
    }

    if(grepl('_R1_', x)){
      system(paste0('scp ', remoteHost, ':', x, ' ', paste0(outputDir, '/Data/Undetermined_S0_L001_R1_001.fastq.gz')))
    }

    if(grepl('_R2_', x)){
      system(paste0('scp ', remoteHost, ':', x, ' ', paste0(outputDir, '/Data/Undetermined_S0_L001_R2_001.fastq.gz')))
    }
  }))
