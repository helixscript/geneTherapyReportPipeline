...
### args <- '210409_MN01490_0013_A000H3FJYG'
if(length(args) == 0) stop('Error - no sequencing run id was provided.')
seqRun <- args[1]

system('rm -rf intSiteCaller_vectors')
system('scp -r everett@microb120.med.upenn.edu:/media/lorax/data/protected/vectorSequences ./intSiteCaller_vectors')
...
for(refGenome in unique(s$refGenome)){
  ...
  message('runIntSiteCaller(', paste0('output/', seqRun, '_', refGenome), ',', refGenome, ',', s, ')')
