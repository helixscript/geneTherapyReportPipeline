...
This report provides an initial overview of the performance of the run specified in the title. In the first table, the report specifies the most prevalent bar codes detected. The displays on the following page show examples of the sequences detected. Following the sequence heat maps, a table shows the accession numbers, and the reads for each aligned to the human genome and yeast genome. The alignment to the yeast genome allows detection of positive controls, which consist of sythentic lentivirus LTRs joined to sequences from the Saccharomyces cereviseae genome. This allows assessment of the success of the run while rigorously monitoring possible PCR cross over between specimens. The final table shows the sonic abundance (marked "estAbund") of the yeast positive controls. Recovery of sonic abundance greater than 20 for each site documents robust data generation.
...
if(nrow(LTRsampleTable) > 0){
  ...
  # cat('whereas the following table shows the prevalence of different leader sequences.\n')
  cat('\n\\vspace{0.5cm}\n')
...
if(nrow(runStats) > 0){
  ...
  # Switch all zeros to NA.
  runStats[runStats == 0] <- NA
